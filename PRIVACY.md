...
#### Most of the data that the bot uses are stored after running a command (so by user interaction), everything that is stored after running a command are used to make the bot work. The main function of our bot (counting channel) required the bot to know with channel have to be updated.

#### Currently the bot stores the following data after running some of the commands and are required to make the bot work (server ID, channel ID, custom welcome text, goal information and server settings like prefix and disabled commands). If you use the premium bot and are a Patreon it will also store data about you and this includes (user ID, donate tier, and activated servers in IDs). this is also all the data that can be deleted by you (the user). the last data that is stored after user interaction is vote data and this includes (user ID, total votes, and when you vote for the last time).
...
#### All stored data is used to make the bot work, without channel IDs our bot would have a way to find and update them. The vote data is additional and only gets created when you vote, after that it's used to check if you have permissions to used additional counter.
...
#### All data that has been stored (blacklist data not included) can be deleted and I also tried to make it as easy as possible. If you using the normal bot you can delete the data in 4 ways (kicking and/or banning the bot, deleting the discord server, deleting your discord account and running the command `s/reset`). if you perform one of the actions mention it will permanently delete **ALL** data and there is no way to get your data back.
...
#### Deleted data will never be added back on request, if you lose your data because you perform an action that deletes your data it's really gone. if there is a fatal error in that database or there is a problem that messes up the data and backup could be added back that could also include your data (you can still perform the same actions to get the data deleted). If data has to be added back out of a backup it will be mentioned first so you have time to get the data deleted again.

---

## **4. WILL YOUR INFORMATION BE SHARED WITH ANYONE?**
#### No, we do not share any information and this will stay like that. It's still possible that someone sees the information if you (the user) run the `s/data` command but that's your (the person that runs the command) responsibility to run this command in a private channel.
...
#### Yes, as our products and function have changed it may also bring with it some data store changes. All future changes that may be made surrounding data will be added/remove here. If we change something in here then we will notify users (the user) throw our support server (ServerStats Support) and you can also find the most recent version online (on our Github). If you don't receive a notification do to you not being in the support server then this is our own responsibility.

---

## **7. HOW CAN YOU CONTACT US ABOUT THIS POLICY?**
#### You can always contact me directly throw discord by DMing me (ZixeSea#1234) or joining the [support server](https://discord.com/invite/vE8qKNV) (ServerStats Support). You can also send an email to support@serverstatsbot.com (keep in mind that this could take some time to get a response).
